<?php
	...
	abstract class Opencart_Globepay_Helper extends Controller
	{
		...
		public function m1($order_info,$order_id)
		{
			...
			if (strpos($http_host, '/') !== false) {
				...
				if (!$this->isWeixinClient()) {
					...
					$url = "https://pay.globepay.co/api/v1.0/wechat_jsapi_gateway/partners/{$partner_code}/orders/{$new_order_id}";
				}
		...
		public function m3($order)
		{
			...
			if (empty($order_id) || !in_array($order['payment_code'], array('globepayunionpayexpress','globepayunionpay', 'globepayalipay', 'globepay'))) {
...
class ControllerExtensionPaymentglobepay extends Opencart_globepay_Helper {
	var $id='globepay';
	public function get_notify_url()
	{
	    // TODO Auto-generated method stub
	    return $this->url->link('extension/payment/globepay/notify');
	}
	...
	public function index() {
		...
		$data['payment_submit_url']            = $this->http_to_https($this->url->link('extension/payment/globepay/send','',true));

		return $this->load->view('extension/payment/globepay', $data);
	}

	public function refund(){
	    $this->load->model('checkout/order');
	    $this->language->load('extension/payment/globepay');
	    $request = array(
	...
	public function callback(){
	   $this->load->model('checkout/order');
	    ...
	    $wait_status = $this->config->get('globepay_order_payWait_status_id');
	    $succeed_status = $this->config->get('globepay_order_succeed_status_id');
	    $result =$this->db->query(
	       "select *
	        from `" . DB_PREFIX . "order`
	...
	public function send() {
	    ...
	    try {
	        ...
	        $this->session->data['cart'] = array();
	        $callback = $this->http_to_https($this->url->link('extension/payment/globepay/callback','',true));
	        ...
	        // $json['success'] = $resArr->pay_url.(strpos($resArr->pay_url, '?')==false?'?':'&')."directpay=true&time=$time&nonce_str=$nonce_str&sign=$sign&redirect=".urlencode($callback);
					$json['success'] = $resArr->pay_url.(strpos($resArr->pay_url, '?')==false?'?':'&')."directpay=true&time=$time&nonce_str=$nonce_str&sign=$sign&redirect=".$callback;
					$this->response->addHeader('Content-Type: application/json');
