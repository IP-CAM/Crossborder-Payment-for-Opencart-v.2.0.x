<?php
	...
	$this->load->model('checkout/order');
	    ...
	    $wait_status = $this->config->get('globepayalipay_order_payWait_status_id');
	    $succeed_status = $this->config->get('globepayalipay_order_succeed_status_id');
	    $result =$this->db->query(
	       "select *
	        from `" . DB_PREFIX . "order`
	    ...
	    if($result->num_rows){
	        //再次检查下是否已支付
	        $partner_code = $this->config->get('globepayalipay_account');
	        $credential_code = $this->config->get('globepayalipay_secret');
